/**
 * PD160 Line Decoder
 * Decodes a single PD160 scan line producing TWO pixel rows
 * PD160 uses same dual-luminance YUV encoding as PD120/PD180: Y-even + V-avg + U-avg + Y-odd
 * Resolution: 512×400 (balanced between PD120's 640×496 and quality)
 * Pixel dwell time: 382µs per pixel (between PD120's 190µs and PD180's 286µs)
 */
...
export interface DecodedLine {
  ...
  width: number;
  height: number; // Always 2 for PD160 (two rows per scan line)
  isOddLine: boolean; // Not used for PD160, always false
}

export class PD160LineDecoder {
  private lowPassFilter: ExponentialMovingAverage;

  private readonly horizontalPixels = 512;
  private readonly verticalPixels = 400;
  ...
  constructor(sampleRate: number) {
    // PD160 timing (in seconds)
    const syncPulseSeconds = 0.020;      // 20ms sync pulse
    const syncPorchSeconds = 0.00208;    // 2.08ms porch after sync
    const channelSeconds = 0.195584;     // 195.584ms per channel (all 4 channels same duration)

    // Total scan line: sync(20ms) + porch(2.08ms) + 4 × channel(195.584ms) = 804.416ms
  ...
  /**
   * Decode a single PD160 scan line producing TWO pixel rows
   ...
   * @returns Decoded line data (TWO rows for PD160: even + odd)
   */
  decodeScanLine(
    ...
    if (syncPulseIndex + this.endSamples > scanLineBuffer.length) {
      console.warn(`PD160: Buffer too short for full line decode`);
      return null;
    }
    ...
    return {
      ...
      height: 2, // PD160 returns TWO rows per scan line
      isOddLine: false,
    };
  }
