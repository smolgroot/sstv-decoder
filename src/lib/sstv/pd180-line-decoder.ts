/**
 * PD180 Line Decoder
 * Decodes a single PD180 scan line producing TWO pixel rows
 * PD modes transmit: Y-even + V-avg + U-avg + Y-odd (4 channels per scan line)
 * PD180 has longer pixel time than PD120 (286µs vs 190µs) for higher quality
 */
...
export interface DecodedLine {
  ...
  width: number;
  height: number; // Always 2 for PD180 (two rows per scan line)
  isOddLine: boolean; // Not used for PD180, always false
}

export class PD180LineDecoder {
  ...
  constructor(sampleRate: number) {
    // PD180 timing (in seconds)
    const syncPulseSeconds = 0.020;      // 20ms sync pulse
    const syncPorchSeconds = 0.00208;    // 2.08ms porch after sync
    const channelSeconds = 0.1824;       // 182.4ms per channel (all 4 channels same duration)
    // PD180 pixel time: 182.4ms / 640 pixels = 285µs per pixel (vs PD120: 190µs)

    // Total scan line: sync(20ms) + porch(2.08ms) + 4 × channel(182.4ms) = 751.68ms
   ...
   * Uses ITU-R BT.601 color space conversion (same as Robot36 and PD120)
   */
  private yuv2rgb(y: number, ry: number, by: number): { r: number; g: number; b: number } {
    // Use the same YUV to RGB conversion as Robot36 and PD120
  ...
  /**
   * Decode a single PD180 scan line producing TWO pixel rows
   ...
   * @returns Decoded line data (TWO rows for PD180: even + odd)
   */
  decodeScanLine(
    ...
    if (syncPulseIndex + this.endSamples > scanLineBuffer.length) {
      console.warn(`PD180: Buffer too short for full line decode`);
      return null;
    }
    ...
    return {
      ...
      height: 2, // PD180 returns TWO rows per scan line
      isOddLine: false,
    };
  }
